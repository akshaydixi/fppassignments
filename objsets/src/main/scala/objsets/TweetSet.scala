...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def descendingByRetweetIter(tweetSet: TweetSet, list: TweetList): TweetList = {
      if (tweetSet.isInstanceOf[Empty]) {
        list
      } else {
        def tweet = tweetSet.leastRetweeted
        descendingByRetweetIter(tweetSet.remove(tweet), new Cons(tweet, list))
      }
    };
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def leastRetweeted: Tweet = {
    ...
    def rightTweet = if (right.isInstanceOf[Empty]) elem else right.leastRetweeted
    if (leftTweet.retweets < rightTweet.retweets)
      if (leftTweet.retweets < elem.retweets)
        leftTweet
      else
        elem
    else 
      if (rightTweet.retweets < elem.retweets)
        rightTweet
      else
        elem
  }
